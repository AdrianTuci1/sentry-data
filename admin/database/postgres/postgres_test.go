...
func testOrgsWithPagination(t *testing.T, db database.DB) {
	...
	user, err := db.InsertUser(ctx, &database.InsertUserOptions{Email: "test@statsparrot.com"})
	require.NoError(t, err)
	require.Equal(t, "test@statsparrot.com", user.Email)
...
func testOrgsMembersPagination(t *testing.T, db database.DB) {
	...
	adminUser, err := db.InsertUser(ctx, &database.InsertUserOptions{Email: "test1@statsparrot.com", DisplayName: "John Admin"})
	require.NoError(t, err)

	viewerUser, err := db.InsertUser(ctx, &database.InsertUserOptions{Email: "test2@statsparrot.com", DisplayName: "Jane Viewer"})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.NoError(t, db.InsertOrganizationInvite(ctx, &database.InsertOrganizationInviteOptions{Email: "test3@statsparrot.com", InviterID: adminUser.ID, OrgID: org.ID, RoleID: viewer.ID}))

	// fetch members without name filter
	users, err := db.FindOrganizationMemberUsers(ctx, org.ID, "", true, "", 1, "")
	require.NoError(t, err)
	require.Equal(t, len(users), 1)
	require.Equal(t, "test1@statsparrot.com", users[0].Email)

	// fetch members with name filter
	users, err = db.FindOrganizationMemberUsers(ctx, org.ID, "", true, users[0].Email, 1, "")
	require.NoError(t, err)
	require.Equal(t, len(users), 1)
	require.Equal(t, "test2@statsparrot.com", users[0].Email)
	...
	require.Equal(t, "test3@statsparrot.com", invites[0].Email)
...
func testManagedGitRepos(t *testing.T, db database.DB) {
	...
	user, err := db.InsertUser(context.Background(), &database.InsertUserOptions{Email: fmt.Sprintf("user%d@statsparrot.com", time.Now().UnixNano())})
	require.NoError(t, err)
...
func testOrganizationInviteAttributes(t *testing.T, db database.DB) {
	...
	email := "invitee-attrs@statsparrot.com"
	attributes := map[string]any{"attr1": "value1", "attr2": "value2"}
	...
	t.Run("InsertOrganizationInvite without attributes normalizes to empty map", func(t *testing.T) {
		email2 := "invitee-no-attrs@statsparrot.com"
		err := db.InsertOrganizationInvite(ctx, &database.InsertOrganizationInviteOptions{
			Email:  email2,
			OrgID:  org.ID,
			RoleID: role.ID,
	...
	t.Run("InsertOrganizationInvite rejects invalid attributes", func(t *testing.T) {
		err := db.InsertOrganizationInvite(ctx, &database.InsertOrganizationInviteOptions{
			Email:      "invitee-invalid-attrs@statsparrot.com",
			OrgID:      org.ID,
			RoleID:     role.ID,
...
func testAttributeValidation(t *testing.T, db database.DB) {
	...
	user, err := db.InsertUser(ctx, &database.InsertUserOptions{Email: "test-validation@statsparrot.com"})
	require.NoError(t, err)
	...
	t.Run("InsertOrganizationMemberUser validation", func(t *testing.T) {
		user2, err := db.InsertUser(ctx, &database.InsertUserOptions{Email: "test2-validation@statsparrot.com"})
		require.NoError(t, err)
...
func seed(t *testing.T, db database.DB) (orgID, projectID, userID string) {
	...
	// create a user with random email id
	adminUser, err := db.InsertUser(ctx, &database.InsertUserOptions{Email: fmt.Sprintf("user%d@statsparrot.com", time.Now().UnixNano())})
	require.NoError(t, err)
