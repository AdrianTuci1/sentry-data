...
type ParrotPeriodToDateTuple = typeof RILL_PERIOD_TO_DATE;
export type ParrotPeriodToDate = ParrotPeriodToDateTuple[number];

type ParrotPreviousPeriodTuple = typeof RILL_PREVIOUS_PERIOD;
export type ParrotPreviousPeriod = ParrotPreviousPeriodTuple[number];

type ParrotLatestTuple = typeof RILL_LATEST;
...
export function isParrotPreviousPeriod(
  ...
  return RILL_PREVIOUS_PERIOD.includes(value as ParrotPreviousPeriod);
}

export function isParrotPeriodToDate(value: string): value is ParrotPeriodToDate {
  return RILL_PERIOD_TO_DATE.includes(value as ParrotPeriodToDate);
}
...
const defaultBuckets: RangeBuckets = {
  latest: RILL_LATEST.map((r) => parseParrotTime(r)),
  periodToDate: RILL_PERIOD_TO_DATE.map((r) => parseParrotTime(r)),
  previous: RILL_PREVIOUS_PERIOD.map((r) => parseParrotTime(r)),
  custom: [],
  allTime: false,
};
