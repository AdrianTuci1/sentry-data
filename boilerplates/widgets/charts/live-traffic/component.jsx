...
const LiveTrafficChart = ({ data = {}, isMock = false }) => {
    const points = (Array.isArray(data.dataPoints) && data.dataPoints.length > 0 ? data.dataPoints : null) || 
                   (Array.isArray(data.data) && data.data.length > 0 ? data.data : null) || 
                   [60, 50, 70, 65, 80, 75, 90, 85, 100, 95, 110, 100, 120];

    const value = data.value ?? data.data?.value ?? "950";
    const unit = data.unit ?? data.data?.unit ?? "usr";
    ...
    return (
        <div style={{ position: 'relative', width: '100%', height: '100%' }}>
            <div style={{ position: 'absolute', top: '-10px', left: 0, zIndex: 10, display: 'flex', alignItems: 'baseline', gap: '4px' }}>
                ...
                <span style={{ fontSize: '12px', color: '#E5E7EB' }}>{unit}</span>
            </div>
