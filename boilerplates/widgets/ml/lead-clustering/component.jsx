...
// This should technically be done early on, but here is fine since it runs once
if (!echarts.getTransform('ecStat:clustering')) {
    echarts.registerTransform(ecStat.transform.clustering);
}

const LeadClustering = ({ data = {}, isMock = false }) => {
    // Domain data: [Engagement Score (0-100), Conversion Probability (0.1-1.0), Name]
    const sourceData = (Array.isArray(data.clusteringData) && data.clusteringData.length > 0 ? data.clusteringData : null) ||
    ...
    const option = {
        ...
        tooltip: {
            ...
            formatter: (params) => {
                ...
                if (!dataRow) return '';
                const eng = dataRow[0] || 0;
                const prob = dataRow[1] || 0;
                const name = dataRow[2] || 'Unknown';
                return `
    ...
    return (
        <div className="lead-clustering-wrapper">
            {/* Shrunken Chart */}
            <div className="lead-clustering-chart">
                <ReactECharts option={option} style={{ height: '100%', width: '100%' }} />
            </div>

            {/* Latest Leads List below */}
            <div className="latest-leads-list">
                <h4 className="latest-leads-title">Latest Leads</h4>
                <div className="latest-leads-grid">
                    {sourceData.slice(0, 12).map((lead, i) => {
                        const prob = lead[1] || 0;
                        ...
                        return (
                            <div key={i} className="lead-item" style={{ borderLeft: `2px solid ${statusColor}` }}>
                                <div className="lead-item-col">
                                    <span className="lead-item-name">{lead[2]}</span>
                                    <span className="lead-item-eng">{lead[0]}% Eng.</span>
                                </div>
                                <span className="lead-item-status" style={{
                                    color: statusColor,
