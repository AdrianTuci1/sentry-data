...
# Parrot 0.60 - Multiple metrics in leaderboards and leaderboards in Canvas!
...
![release-0 60](<https://cdn.statsparrot.com/docs/release-notes/release-060.gif>)
...
- Added metrics view and dashboards to `statsparrot project refresh` so external tables also refreshed.
...
- Fixed issue when up to date GitHub Repository failed to connect to Parrot Cloud project. 
- Fixed bug in new filter when newline / comma present.
- Fixed issue where Google Drive backed Big Query Table not able to load in Parrot.
