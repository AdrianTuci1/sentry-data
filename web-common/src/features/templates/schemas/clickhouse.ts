...
export const clickhouseSchema: MultiStepFormSchema = {
  ...
  properties: {
    ...
    write_dsn: {
      ...
      "x-placeholder": "clickhouse://localhost:9000?username=default",
      "x-secret": true,
      "x-visible-if": { deployment_type: ["cloud", "self-managed"] },
      "x-step": "connector",
      "x-advanced": true,
    ...
    query_settings: {
      ...
      description:
        "Default query settings (comma-separated, e.g. max_threads=8)",
      "x-placeholder": "max_threads=8",
      "x-visible-if": { deployment_type: ["cloud", "self-managed"] },
      "x-step": "connector",
      "x-advanced": true,
    },
    query_settings_override: {
      ...
      description: "Override query settings that take precedence over defaults",
      "x-placeholder": "max_threads=8",
      "x-visible-if": { deployment_type: ["cloud", "self-managed"] },
      "x-step": "connector",
      "x-advanced": true,
    ...
    dial_timeout: {
      type: "string",
      title: "Dial timeout",
      description: "Connection dial timeout duration (e.g. 60s)",
      "x-placeholder": "60s",
      "x-visible-if": { deployment_type: ["cloud", "self-managed"] },
      "x-step": "connector",
      "x-advanced": true,
    ...
    read_timeout: {
      type: "string",
      title: "Read timeout",
      description: "Read timeout duration (e.g. 300s)",
      "x-placeholder": "300s",
      "x-visible-if": { deployment_type: ["cloud", "self-managed"] },
      "x-step": "connector",
      "x-advanced": true,
