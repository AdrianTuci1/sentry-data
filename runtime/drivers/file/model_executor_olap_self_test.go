...
func TestExecute(t *testing.T) {
	...
	t.Run("test_parquet_export", func(t *testing.T) {
		...
		compareResult, err := olap.Query(context.Background(), &drivers.Statement{
			Query: fmt.Sprintf(`
				...
				expected AS (SELECT * FROM read_parquet('testdata/expected_export/expected_output.parquet')),
				comparison AS (
		...
		require.Equal(t, "MATCH", comparisonResult, "Exported data does not match expected data")
	...
	t.Run("test_csv_export", func(t *testing.T) {
		...
		compareResult, err := olap.Query(context.Background(), &drivers.Statement{
			Query: fmt.Sprintf(`
				...
				expected AS (SELECT * FROM read_csv('testdata/expected_export/expected_output.csv')),
				comparison AS (
					SELECT COUNT(*) as mismatch_count
					FROM (
						SELECT * FROM actual
	...
	t.Run("test_xlsx_export", func(t *testing.T) {
		...
		compareResult, err := olap.Query(context.Background(), &drivers.Statement{
			Query: fmt.Sprintf(`
				...
				expected AS (SELECT * FROM st_read('testdata/expected_export/expected_output.xlsx')),
				comparison AS (
					SELECT COUNT(*) as mismatch_count
					FROM (
						SELECT * FROM actual
		...
		require.True(t, compareResult.Next())
		require.NoError(t, compareResult.Scan(&comparisonResult))
		require.Equal(t, "MATCH", comparisonResult, "Exported XLSX data does not match expected data")
		require.NoError(t, compareResult.Close())
	})

	t.Run("test_invalid_format", func(t *testing.T) {
	...
	t.Run("test_missing_sql", func(t *testing.T) {
		outPath := filepath.Join(tempDir, "out.parquet")
		execOpts := &drivers.ModelExecuteOptions{
			ModelExecutorOptions: opts,
			InputProperties:      map[string]any{},
