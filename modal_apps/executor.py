...
AGENT_MANAGER_PATH = REPO_ROOT / "boilerplates" / "manager" / "agent_manager.py"
...
image = modal.Image.debian_slim() \
    ...
    .add_local_file(str(AGENT_MANAGER_PATH), "/root/agent_manager.py")
...
def main():
    """
    Permite rularea: modal run modal_apps/executor.py
    Utilitate: Verifică dacă aplicația Modal este configurată corect.
    """
    print("✅ Modal App 'sentry-sandbox-executor' is ready.")
    print("👉 To deploy as a permanent API: modal deploy modal_apps/executor.py")
    print("👉 To run in dev mode (hot-reload): modal serve modal_apps/executor.py")
