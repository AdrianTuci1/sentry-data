...
test.describe("canvas time filters", () => {
  ...
  test("save default filters button works", async ({ page }) => {
    ...
    expect(page.url()).toContain(
      "?tr=PT24H&compare_tr=statsparrot-PP&f.AdBids_metrics=domain+IN+%28%27facebook.com%27%2C%27google.com%27%2C%27msn.com%27%29",
    );
  ...
  test("legacy filters without prefix still work", async ({ page }) => {
    ...
    await page.goto(
      `${currentUrl}?tr=PT24H&compare_tr=statsparrot-PP&f=domain+IN+%28%5B%27facebook.com%27%2C%27google.com%27%2C%27msn.com%27%5D%29`,
    );
