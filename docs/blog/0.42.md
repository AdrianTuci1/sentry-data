...
# Parrot 0.42 - Alerts, Bookmarks, Custom APIs
...
![release-0 42](<https://storage.googleapis.com/prod-cdn.statsparrot.com/docs/release-notes/0.41_bookmarks_alerts.gif>)
...
For more details on configuration and use cases [check out our alerts page](/guide/alerts).
...
More details on [bookmarks here](/guide/dashboards/bookmarks).
...
We're always looking for more ways to bring data into Parrot. This release includes a connector for Redshift - [more details here](/developers/build/connectors/data-source/redshift) on how to set up credentials and create sources from your Redshift data.
...
- Adding support for configuring the default OLAP engine directly through `statsparrot.yaml`.
...
- Added hardening to prevent runtime panics from occurring when there is a missing `statsparrot.yaml` in the project.
...
- Added support for the `statsparrot env push` and `statsparrot env pull` CLI commands to push variables / credentials from a local `.env` to a cloud project and pull already defined variables / credentials to a local `.env` (which is part of `.gitignore`).
...
- Fixed bug where a relative duckdb database file that was included in a subdirectory of the Git project couldn't be used by the DuckDB connector when the project has been deployed to Parrot Cloud.
...
- Added tooltip in the Parrot Cloud UI to display the full error message when hovering on the Status page.
