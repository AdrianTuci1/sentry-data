<script lang="ts">
  ...
  import { createAdminServiceUnsubscribeReportUsingToken } from "@statsparrot/web-admin/features/scheduled-reports/unsubscribe-report-using-token.ts";
...
<CtaLayoutContainer>
  <CtaContentContainer>
    <div class="flex flex-col gap-y-2">
      {#if error}
        <h2 class="text-lg font-semibold">{m.report_unsubscribe_failed()}</h2>
        <CtaMessage>
          {error}
        </CtaMessage>
      {:else if loading}
        <h2 class="text-lg font-semibold">{m.report_unsubscribing()}</h2>
      {:else}
        <h2 class="text-lg font-semibold">{m.report_unsubscribed()}</h2>
      {/if}
    </div>
  </CtaContentContainer>
</CtaLayoutContainer>
