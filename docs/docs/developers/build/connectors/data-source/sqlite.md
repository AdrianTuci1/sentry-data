...
If you plan to deploy the project to Parrot Cloud, place the SQLite database file in a `data` folder in your Parrot project directory and use the relative path (e.g., `data/mydatabase.db`).

:::

## Deploy to Parrot Cloud

Once a project with a SQLite source has been deployed using `statsparrot deploy`, Parrot Cloud will need to be able to access and retrieve the underlying database file. In most cases, this means that the corresponding SQLite database file should be included within a directory in your Git repository, which will allow you to specify a relative path in your source definition (from the project root).

:::warning When Using An External SQLite Database

If the SQLite database file is external to your Parrot project directory, you will still be able to use the fully qualified path to read this SQLite database _locally_ using Parrot Developer. However, when deployed to Parrot Cloud, this source will throw an **error**.
