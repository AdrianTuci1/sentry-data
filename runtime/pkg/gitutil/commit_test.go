...
func TestCommitAndForcePush(t *testing.T) {
	...
	author := Signature{Name: "Parrot", Email: "noreply@statsparrot.com"}
...
func TestUserSignature(t *testing.T) {
	...
	t.Run("returns the configured identity", func(t *testing.T) {
		path := setupTestRepository(t) // setupGitConfig sets Test User <test@statsparrot.com>
		sig, err := UserSignature(ctx, path)
		require.NoError(t, err)
		require.Equal(t, Signature{Name: "Test User", Email: "test@statsparrot.com"}, sig)
	})
