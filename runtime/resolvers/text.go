		...
		if r.props.UseFormatTokens {
			...
			for _, row := range rows {
				for field, val := range row {
					...
					row[field] = fmt.Sprintf("__STATSPARROT__FORMAT__(%s)", string(data))
				}
			}
		}
...
// textFormatToken is the payload inside a __STATSPARROT__FORMAT__(...) token generated by the text resolver.
