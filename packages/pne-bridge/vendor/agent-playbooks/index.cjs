...
exports.planMlModels = exports.recommendNextSteps = exports.buildFirstRunPlaybook = exports.emptySessionState = void 0;
...
exports.emptySessionState = emptySessionState;
const buildFirstRunPlaybook = (input) => {
    const session = {
        ...(input.sessionState || (0, exports.emptySessionState)()),
        lastUpdatedAt: nowIso()
    };
...
exports.buildFirstRunPlaybook = buildFirstRunPlaybook;
const recommendNextSteps = (input) => {
    const session = {
        ...(input.sessionState || (0, exports.emptySessionState)()),
        lastUpdatedAt: nowIso()
    };
...
exports.recommendNextSteps = recommendNextSteps;
const planMlModels = (input) => {
    const candidates = [];
...
exports.planMlModels = planMlModels;
const finalize = (session, result) => ({
    ...result,
    sessionState: {
        ...session,
...
const chooseBestRecipe = (setupGuide, prereqs) => {
    if (!setupGuide?.recipes?.length) {
        return undefined;
    }
    const binaries = prereqs?.binaries || {};
    if (binaries.bq)
        return setupGuide.recipes.find((recipe) => recipe.connectorType === 'bigquery');
    ...
    if (binaries.duckdb)
        return setupGuide.recipes.find((recipe) => recipe.connectorType === 'duckdb');
    if (binaries.snowsql)
        return setupGuide.recipes.find((recipe) => recipe.connectorType === 'snowflake');
    if (binaries.psql)
        return setupGuide.recipes.find((recipe) => recipe.connectorType === 'postgres');
    return setupGuide.recipes.find((recipe) => recipe.connectorType === 'hosted') || setupGuide.recipes[0];
};
const prerequisitesReady = (prereqs) => {
    if (!prereqs) {
        return true;
    }
    return Object.values(prereqs.binaries || {}).some(Boolean) || Object.values(prereqs.env || {}).some(Boolean);
};
const hasHostedConnector = (connectors) => (connectors.some((connector) => connector.type === 'hosted'));
const explainFirstRunState = (step, connectorType) => {
...
const buildRecipeQuestion = (recipe) => (`Do you want to follow the ${recipe.title} setup path?`);
...
const rankMlCandidates = (candidates, domain, question) => {
    const normalizedQuestion = (question || '').toLowerCase();
    const domainBias = (candidate) => {
        if (domain === 'ecommerce' && candidate.taskType === 'forecasting')
            return 0.08;
        if (domain === 'cybersecurity' && candidate.taskType === 'anomaly_detection')
            return 0.12;
        return 0;
    };
    const questionBias = (candidate) => {
        if (normalizedQuestion.includes('forecast') && candidate.taskType === 'forecasting')
            return 0.1;
        if (normalizedQuestion.includes('anomaly') && candidate.taskType === 'anomaly_detection')
            return 0.1;
        if (normalizedQuestion.includes('predict') && ['classification', 'regression'].includes(candidate.taskType))
            return 0.08;
        return 0;
    };
    return [...candidates]
