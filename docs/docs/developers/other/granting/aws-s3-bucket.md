...
* Create a new bucket, in the US Standard S3 region, named `statsparrot-ORG_NAME-share`, where **"ORG_NAME"** is your organizational name. Make sure the bucket name is a DNS-compliant address. For example, the name should not include any underscores; use hyphens instead, as shown in the example above.
...
We can provide access to the desired buckets using the bucket policies. You can provide access to the root user for Parrot Data AWS Account. 
...
1. Add the following bucket policy for the access: (Replace statsparrot-ORG_NAME-share or bucket-to-be-shared with the bucket name)
   ...
   "Statement": [
       {
           ...
           "Resource": [
               "arn:aws:s3:::statsparrot-ORG_NAME-share/*"
           ]
       },
       {
           "Effect": "Allow",
...
```shell
export BUCKET="statsparrot-ORG_NAME-share" #Create a bucket with Name of Bucket to be shared

curl -s http://pkg.statsparrot.com/aws/cross-account-s3-bucket-policy.json | sed "s/bucket-to-be-shared/${BUCKET}/" | tee policy.json
...
We can provide access to the S3 Bucket through an IAM Role which will be assumed by the Parrot Data AWS Account to gain the access to the buckets. 
...
2. Use Amazon S3 URL: 
`https://s3.amazonaws.com/cf-templates.statsparrot.com/statsparrot-s3-bucket-access.yaml`
...
** Stack Name: `statsparrot-s3-access`
...
Description: 'AWS CloudFormation Template for providing Parrot Data Access to S3 Bucket. It creates a
  Role that can be assumed by the Parrot AWS Account. The Role has a IAM policy associated with them.'
...
Resources:
  S3Role:
    Type: AWS::IAM::Role
    Properties:
      Description: 'Parrot Access to the S3 Bucket. Managed by: Cloudformation'
...
Outputs:
  RoleName:
    ...
    Description: S3 Access Role Arn, to be shared with Parrot
  ExternalID:
    Value: !Ref ExternalID
    Description: ExternalID for Secured Access, to be shared with Parrot
```

## S3 Logging

As part of your data uploads, Parrot Data requires logging to be enabled for the data you intend on sending to us for processing. Parrot Data may use these logs for troubleshooting, pipeline optimization, and/or billing. In order to allow Parrot Data to access these logs, please follow the instructions below to set up the proper configuration.

If you have not enabled logging for the bucket Parrot Data is ingesting data from, please follow these steps:

1. Go to http://aws.amazon.com
    * Login to your AWS account on the AWS Management Console and navigate to the S3 tab
    * Click the “Properties” icon on the upper-right menu to bring up the “Properties” options on the bucket Parrot Data is ingesting data from
    * Under “Logging”, click “Enabled”
    * Under “Target Bucket”, select the same name of the bucket Parrot Data is processing data from
...
3. Click on "Create Stack" and under "Choose a template", either upload the template provided by Parrot Data, or use the following s3URL: https://s3.amazonaws.com//cf-templates.statsparrot.com/ParrotS3LogRole.yaml

4. The template allows you to set the following parameters:
    * **"Stack name"**
      * Name the stack `statsparrot-ORG_NAME-s3Logs`
    * **"S3LogBucketName"**
...
6. Once the stack has finished being created, send your Parrot Data representative the results on the "Outputs" tab.
    * **RoleARN**: This IAM role identifier allows Parrot Data to ingest the S3 Logs
    * **S3BucketName**: This is the name of S3 Bucket that the Parrot Data role can have access to, in this case, it should be the bucket name you inputted into Cloudformation
