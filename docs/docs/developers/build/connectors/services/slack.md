...
Once the Slack integration has been set up, the Slack destination will need to be enabled on a per-project basis (note - alerts can only be configured on projects deployed to Parrot Cloud). This requires the `SLACK_BOT_TOKEN` connector variable to be set, which can be configured in Parrot in a manner very similar to [setting credentials](/developers/deploy/deploy-credentials) for other connectors. Please use one of the available options below.
...
Within your project's `.env` file (i.e. `<STATSPARROT_PROJECT_HOME>/.env`), you can set this connector variable with the Slack Bot User OAuth Token:
...
Afterward, if the project has already been deployed to Parrot Cloud, you can `statsparrot env push` to update your cloud deployment accordingly.

### Using the `statsparrot env set` command

Another option to set this connector variable within your project is to use the `statsparrot env set` command, i.e.:

```shell
statsparrot env set SLACK_BOT_TOKEN <BOT_USER_OAUTH_TOKEN>
```

Afterward, if the project has already been deployed to Parrot Cloud, you can `statsparrot env push` to update your cloud deployment accordingly.

### Enabling the Slack connector through `statsparrot.yaml`

You can enable the Slack "connector" within your project by updating your project's `statsparrot.yaml` file with the following configuration:

```yaml
# Rest of your statsparrot.yaml contents
connectors:
- name: slack
  type: slack
```
...
When deploying a project to Parrot Cloud, Parrot requires you to explicitly provide Slack credentials used in your project. Please refer to our [connector YAML reference docs](/reference/project-files/connectors#slack) for more information.
