...
User tokens (also called personal access tokens or PATs) provide programmatic access to Parrot Cloud tied to your personal user account. They inherit your user permissions and are ideal for local development, scripting, and integrations like MCP (Model Context Protocol).

## Overview

User tokens are designed for:
- **Local development** - Testing and developing with Parrot APIs from your machine
...
- **Experimentation** - Trying out Parrot APIs without production concerns
- **CLI authentication** - Authenticating Parrot CLI commands
...
```bash
statsparrot token issue --display-name "Local Development"
```
...
statsparrot token issue --display-name "MCP Token" --ttl-minutes 1440

# Expires in 7 days (10080 minutes)
statsparrot token issue --display-name "Testing Token" --ttl-minutes 10080
```
...
```bash
statsparrot token revoke <token-id>
```

Or revoke by display name:

```bash
statsparrot token revoke --display-name "Local Development"
```
...
User tokens can be used to authenticate requests to Parrot's custom APIs:

```bash
curl https://api.statsparrot.com/v1/organizations/<org>/projects/<project>/runtime/api/<api-name> \
