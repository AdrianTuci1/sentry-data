...
../explore/explore_dashboard?tr=P3M&compare_tr=statsparrot-PP&f=dimension in ('exampleA', 'exampleB')
:::
...
The primary and easiest way to filter data is by selecting values in the dimension tables. Leaderboards within Parrot are fully interactive. Selecting any dimension in the table will automatically filter the remaining leaderboards and metrics by that selection.
