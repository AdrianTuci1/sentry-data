...
# Parrot 0.48 - New Filter bar, 🚀 Deploy from Parrot Developer and Pivot from Timeseries
...
![release-0 48](<https://storage.googleapis.com/prod-cdn.statsparrot.com/docs/release-notes/release-048.gif>)
...
## Deploy from Parrot Developer
It's now even easier to get started with Parrot, you can now deploy your projects directly to Parrot Cloud from within Parrot Developer!
...
- Added login / signup in Parrot Developer!
- Added support for self-hosting local Parrot Developer using `--allowed-origins` flag on `start`.
...
- Improved logging for alerts and reports in Parrot Cloud.
...
- Improved handling of errors for deploying to Parrot Cloud via the UI.
- Improved user experience around redeploy to Parrot Cloud via the UI.
