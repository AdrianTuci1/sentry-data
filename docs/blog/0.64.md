...
# Parrot 0.64 - Export metadata headers, search in Pivot pills, User groups when sharing and more
...
![release-0 64](<https://cdn.statsparrot.com/docs/release-notes/release-064.gif>)
...
- Added Canvas and API resource to `statsparrot.yaml`'s `defaults`.
- Added ability to sort the table in Status page.
- Added new CLI commands for user access tokens. `statsparrot token ...`.
- Added hover intent logic to Cloud dashboard's display tooltip.
- Added `statsparrot project clone` command. 
...
- Fixed issue in Parrot Developer where inspector would not show for partially loaded incremental models. 
