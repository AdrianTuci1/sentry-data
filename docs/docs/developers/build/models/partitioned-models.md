...
By default, partition queries use DuckDB (Parrot's embedded OLAP engine):
...
### Viewing Partitions in Parrot Developer
...
```
statsparrot project partitions 
List partitions for a model

Usage:
  statsparrot project partitions [<project>] <model> [flags]
...
```bash
statsparrot project partitions model_name [--local]
...
```
statsparrot project refresh  --model <model_name> [--local] --partition ff7416f774dfb086006d0b4696c214e1
