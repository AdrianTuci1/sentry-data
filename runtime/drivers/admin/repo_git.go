...
func (r *gitRepo) pullInner(ctx context.Context, userTriggered, force bool) error {
	...
	if !isRepoRoot(r.repoDir) {
		...
		if r.editableDepl {
			...
			err = setGitConfig(r.repoDir, "user.email", "noreply@staticlabs.com")
			if err != nil {
				return err
			}
		}
...
func (r *gitRepo) commitToDefaultBranch(ctx context.Context, message string, force bool) error {
	...
	_, err := gitutil.CommitAll(ctx, r.repoDir, r.subpath, message, gitutil.Signature{Name: "Parrot", Email: "noreply@staticlabs.com"})
	if err != nil {
...
func (r *gitRepo) mergeToBranch(ctx context.Context, branch string, force bool) (resErr error) {
	...
	_, err := gitutil.CommitAll(ctx, r.repoDir, r.subpath, "Auto commit before merging to "+branch, gitutil.Signature{Name: "Parrot", Email: "noreply@staticlabs.com"})
