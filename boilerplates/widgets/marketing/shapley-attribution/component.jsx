...
if (!echarts.getTransform('ecSimpleTransform:aggregate')) {
    echarts.registerTransform(aggregate);
}

const ShapleyAttribution = ({ data = {}, isMock = false }) => {
    // Generate raw attribution data over multiple intervals (simulating simulations)
    // Format: [Channel, Score, Interval]
    const rawData = (Array.isArray(data.rawData) && data.rawData.length > 0 ? data.rawData : null) || 
                        ...
                        channels.forEach(channel => {
                            ...
                            for (let year = 2020; year <= 2025; year++) {
                                for (let i = 0; i < 5; i++) {
                                    ...
                                    genData.push([channel, Math.max(0, score), year]);
                                }
                            }
                        });
                        return genData;
