...
const TechnicalHealth = ({ data = {}, isMock = false }) => {
    const metrics = (Array.isArray(data.metrics) && data.metrics.length > 0 ? data.metrics : null) || 
                    (Array.isArray(data.data?.metrics) && data.data?.metrics.length > 0 ? data.data.metrics : null) || 
                    [
                        { name: 'Uptime', value: '99.9%', status: 'good' },
                        { name: 'Latency', value: '42ms', status: 'good' },
                        { name: 'Errors', value: '0.01%', status: 'good' },
                        { name: 'CPU', value: '84%', status: 'warning' }
                    ];
