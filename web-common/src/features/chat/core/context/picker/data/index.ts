...
import { getMetricsViewPickerOptions } from "@statsparrot/web-common/features/chat/core/context/picker/data/metrics-views.ts";
import { getModelsPickerOptions } from "@statsparrot/web-common/features/chat/core/context/picker/data/models.ts";
...
import { getCanvasesPickerOptions } from "@statsparrot/web-common/features/chat/core/context/picker/data/canvases.ts";
 ...
 * 1. Chooses top level options based on where this is run in statsparrot developer or not.
 *    If statsparrot dev, then sources/models are also included in the top level list along with metrics views and canvases.
...
export function getPickerOptions(
  ...
  const isParrotDev = !get(featureFlags.adminServer);
  ...
  return derived(
    [
      ...
      isParrotDev ? getModelsPickerOptions(client, uiState) : readable(null),
