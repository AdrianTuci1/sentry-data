...
func UploadThumbnailCmd(ch *cmdutil.Helper) *cobra.Command {
	var path string
	var remove bool

	cmd := &cobra.Command{
		Use:   "upload-thumbnail [<org-name> [<path-to-image>]]",
		Args:  cobra.MaximumNArgs(2),
		Short: "Upload a custom thumbnail",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if remove {
				...
				// Confirmation prompt
				if ch.Interactive {
					if err := cmdutil.ConfirmPrompt(fmt.Sprintf("You are removing the custom thumbnail for %q. Continue?", ch.Org), false); err != nil {
						return err
					}
				}
				...
				ch.PrintfSuccess("Removed thumbnail from organization %q\n", ch.Org)
				return nil
			}
			...
			// Confirmation prompt
			if ch.Interactive {
				if err := cmdutil.ConfirmPrompt(fmt.Sprintf("You are changing the custom thumbnail for %q. Continue?", ch.Org), false); err != nil {
					return err
				}
			}
			...
			ch.PrintfSuccess("Updated the thumbnail for %q\n", ch.Org)
			return nil
		},
	}
	...
	cmd.Flags().BoolVar(&remove, "remove", false, "Remove the current thumbnail")
