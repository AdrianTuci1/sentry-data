...
func ResetCmd(ch *cmdutil.Helper) *cobra.Command {
	...
	resetCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !force {
				ch.PrintfWarn("The project will be unavailable for a while as data sources are reloaded from scratch. If you just need to refresh data, use `statsparrot project refresh`.\n")
				if !ch.Interactive {
