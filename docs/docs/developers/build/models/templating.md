...
:::note Parrot Developer is a dev environment

Unless explicitly defined, Parrot Developer will use a `dev` environment. If you want to emulate production locally, you can do so via `statsparrot start --environment prod`.

:::

## Separate Dev and Prod SQL Queries

Another common templating pattern is using the `dev:` partition in your model definitions. This tells Parrot to use a different data source or query when running in development mode, typically with a smaller dataset or different data location.
...
dev:
  sql: "select * from read_parquet('gs://statsparrot-public/github-analytics/Clickhouse/2025/06/commits_2025_06.parquet')"

sql: "select * from read_parquet('gs://statsparrot-public/github-analytics/Clickhouse/*/*/commits_*.parquet')"
```
...
If you use templating in SQL models, you must replace references to tables/models created by other sources or models with `ref` tags. This ensures that the native Go templating engine used by Parrot is able to resolve and correctly compile the SQL syntax during runtime (to avoid any potential downstream errors).

:::

If we simply run Parrot Developer using `statsparrot start`, our model will look like the following (this will also reflect our data model in production, i.e., Parrot Cloud, after we've [pushed the changes for the project to GitHub](/developers/deploy/deploy-dashboard)):

![Variables Example](/img/deploy/templating/vars-example.png)

**Now**, just to illustrate what a local override might look like, let's say we stop Parrot Developer and then restart Parrot via the CLI with the following command:

```bash
statsparrot start --env language="ja" --env local_limit=100
```

Even though we have defaults set in `statsparrot.yaml` (and this will be used by any downstream models and dashboards on Parrot Cloud), we will instead see these local overrides come into effect with our templated logic to return Japanese movies and the model limit is now 100 rows.
