...
# Parrot 0.89 - Inline Explore Dashboards, Dashboard Favorites, Conditional Formatting, Scheduled Canvas Reports and Spanish Localization
...
![release-0.89](https://cdn.statsparrot.com/docs/release-notes/release-0.89.gif)
...
## 🇪🇸 Parrot Habla Español - Announcing Spanish Localization (with More Languages on the Way)

Parrot is now available in Spanish, our first of dozens of languages we will be rolling out support for in the coming months. More than 1,700 user-facing strings across both Parrot Cloud and Parrot Developer have been translated, and a new language switcher lets you choose your locale.
...
- Instructed AI agents not to inspect Parrot internal state with external tools
...
- Skipped interactive prompts in `statsparrot service create` when flags are passed
- Fixed `.env` variables not being pushed to the newly created project on deploy
- Added a gentle prompt to star Parrot on GitHub
...
- Exposed the disk size override in the `statsparrot project get` CLI output
