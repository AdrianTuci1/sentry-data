...
router.get('/health/connectors', context_1.requireAuth, async (req, res) => {
    // Mock Airbyte Status
    const airbyteStatus = {
        name: 'Airbyte Sync',
        status: 'HEALTHY',
    ...
    try {
        await s3_1.s3.listObjects('health-check'); // Minimal check
...
router.get('/scripts', context_1.requireAuth, async (req, res) => {
    const { key } = req.query;
    ...
    try {
        ...
        const url = await s3_1.s3.getSignedDownloadUrl(key);
        res.json({ url });
    }
    catch (e) {
