	...
	removeCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if project != "" {
				...
				ch.PrintfWarn("New users with email addresses ending in %q will no longer automatically be added to project %q of %q. Existing users previously added through this policy will keep their access. (To remove users, use `statsparrot user remove`.)\n", domain, project, ch.Org)
				return nil
			}
			...
			ch.PrintfWarn("New users with email addresses ending in %q will no longer automatically be added to organization %q. Existing users previously added through this policy will keep their access. (To remove users, use `statsparrot user remove`.)\n", domain, ch.Org)
			return nil
		},
	}
